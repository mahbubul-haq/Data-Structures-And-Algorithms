...
///source: https://www.hackerearth.com/practice/data-structures/trees/heapspriority-queues/tutorial/
...
public:
    ...
    void maxHeapify(int i) {
        ...
        if (left <= length and pq[left] > pq[largest])
            largest = left;
        if(right <= length and pq[right] > pq[largest])
            largest = right;
        if (largest != i) {
    ...
    void Increase(int i, int val) {
        if (val < pq[i]) {
            cout << "New value is smaller, can't be inserted" << endl;
            return;
        }
        pq[i] = val;
        while (i > 1 and pq[i / 2] < pq[i]) {
            swap(pq[i / 2], pq[i]);
            i /= 2;
        }
    }
