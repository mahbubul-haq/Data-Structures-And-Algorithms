...
class SegmentTree{
    ...
    void build(int node,int L,int R){
        if(L==R){
            ...
            st[node].first=max(st[left(node)].first,st[right(node)].first);
            st[node].second=0;
        }
    }

    void push(int node){
        st[left(node)].first+=st[node].second;
        st[left(node)].second+=st[node].second;
        st[right(node)].first+=st[node].second;
        st[right(node)].second+=st[node].second;
        st[node].second=0;
    }
