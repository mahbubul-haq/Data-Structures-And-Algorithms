...
#define LSOne(n) (n & (-n))
vi ft[2], ara; //ft[0] for b1 and ft[1] for b2
int n;

void add1(int idx, int val, int id) {
    for (int i = idx ; i <= n; i += LSOne(i)) {
        ft[id][i] += val;
    }
}
...
ll sum(int i, int id) {
    ll su = 0;
    for (int x = i; x > 0; x -= LSOne(x))
        su += ft[id][x];
    return su;
}

ll pref_sum(int idx) {
    return (sum(idx, 0) * idx - sum(idx, 1));
}
