...
int main()
{
    ...
    while(cin>>n>>w>>h>>x>>y){
        ...
        for(int i=0;i<n;i++){
            int a,b,c,d;
            cin>>a>>b>>c>>d;
            ///pair<vector<Point>,vector<Point> > ans=cutPolygon(Point(a,b),Point(c,d),pts);

            if(ccw(Point(a,b),Point(c,d),fountain)) pts=cutPolygon(Point(a,b),Point(c,d),pts);
            else pts=cutPolygon(Point(c,d),Point(a,b),pts);


        }
        cout<<"Case #"<<cas++<<": ";
        cout<<fixed<<setprecision(3)<<area(pts)<<endl;
    }
